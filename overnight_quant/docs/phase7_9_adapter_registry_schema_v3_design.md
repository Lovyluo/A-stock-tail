...
本阶段升级只读来源适配注册表的生命周期表达，并将两个腾讯候选切换为显式只读影子
Provider。它不创建
...
| 槽位 | 含义 | 可否由生产执行 |
|---|---|---|
| `provider_key` | 已选中的显式只读 Provider | 只有状态为 `bound` 且 envelope 精确匹配才可调用；当前仅腾讯两项 |
...
在 B2.2c 中，两行的候选 key 移入 `provider_key`，`candidate_provider_key` 清空，状态改为
`bound`；`legacy_provider_key` 保持不变。该 active key 不能注入到其他 capability、origin、
adapter 或 source version，也不能与 legacy key 相同。
...
公开执行只有在 envelope 类型和 key 都精确匹配时才会调用：
...
错误 key、legacy key、裸 callable 或缺少 envelope 时，门禁在调用前失败并保持
`provider_called=false`。模块不创建默认 envelope 或 transport。私有测试入口仍使用
`test_only` scope，selection hash 根据实际测试矩阵计算，不能冒充生产 hash。
...
B2.2c 只读影子绑定后的 Schema v3 生产 hash：
...
B2.2c 仅完成显式只读影子绑定。默认联网、collector/策略接入、正式来源资格，以及剩余
市场宽度、行业宽度和正式资金流门禁仍须分别审查。
