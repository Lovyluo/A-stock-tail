...
| 14:50 分钟线 | 东财 `trends2` / mootdx 独立候选 | OHLC `元/股`；成交量来源单位；成交额 `元`；东财另含 VWAP | 行中时间为 `event_time`；解析完成为 `available_at` | 东财 `push2_trends2_fields_v2026-07-30`；mootdx `mootdx_0.11.7_tdx_std_bars_1m_v2026-07-31` | 东财四点实测失败 2 次；mootdx 盘后 5 股连通成功，约 966ms、每股 240 条且均含 14:50 | 两个来源均未达到连续 3 日资格门槛，不得拼接证据 |
...
正式分钟来源资格必须连续至少 3 个交易日满足四时点全成功、每点 5 股全覆盖且关键
请求 P95 不超过 2 秒。单日 VERIFIED 只进入 PM 复核队列，不会自动改变配置。市场
