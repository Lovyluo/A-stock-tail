...
| 14:50 分钟线 | mootdx 主资格候选；东财仅独立审计 | OHLC `元/股`；成交量保留来源原生单位；成交额 `元`；mootdx 逐笔另记录成交笔数 | 行中时间为 `event_time`；解析完成为 `available_at`；逐笔聚合 14:49 与 14:50 两个区间 | 东财 `push2_trends2_fields_v2026-07-30`；mootdx 分钟 `mootdx_0.11.7_tdx_std_bars_1m_v2026-07-31`；逐笔 `mootdx_0.11.7_tdx_std_transaction_v2026-08-03` | 东财四点实测失败 2 次；mootdx 2026-08-03 四点覆盖成功但尚无同日逐笔归因 | mootdx 尚未达到连续 3 日 provisional 资格门槛；东财不计入连续合格日；不得拼接证据 |
...
`codex/v041-source-qualification` 增加东财与 mootdx 的来源特定采样入口。每次运行只允许
一个 `--source`，并通过 `--output` 由 Python 原子写入 UTF-8 JSON。2026-07-31 盘后
连通性复核中，mootdx 0.11.7 在约 966ms 内返回 5 股各 240 条分钟记录，5 股均包含
14:50 行；同一时段东财请求约 1233ms 后出现真实 `ProxyError`。这些盘后结果只证明
成功和失败路径可观测，不证明 14:50 SLA。

正式分钟来源资格候选当前仅为 mootdx。它必须连续至少 3 个交易日满足四时点全成功、
每点 5 股全覆盖、14:51 后同源逐笔归因五股一致、provisional 行最终稳定、全部哈希可
重算，且关键请求 P95 不超过 2 秒。东财只保留审计采样，不计入连续合格日。单日
provisional 只进入证据队列，不会自动改变配置。市场
宽度、行业宽度和非代理正式资金流继续分别验证；任一缺失时正式完整股票数仍为 0。
